...
# i assuming the content of list.txt file is output of `ls` command that executed in a folder which contains only `*.md` files.

length = len(titleList)
# we will need index numbers of this list so we should keep the length of this list.

replace = "" # temproary string variable for list item changes.
for i in range(0,length):
    ...
    titleList[i] = replace
tmpList =[] # temproary list variable for list item changes
for i in range(0,length):
...
for i in titleList:
    # this is just writing the final list to output file.
    output.write(i + "\n")
